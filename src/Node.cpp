...
onPortAdded()
{
  ...
  _nodeGeometry.setnSources(nNewIn);
  _nodeState.resizeConnectSet(PortType::In, nNewIn);

  // port Out
  const unsigned int nNewOut = _nodeDataModel->nPorts(PortType::Out);
  _nodeGeometry.setnSinks(nNewOut);
  _nodeState.resizeConnectSet(PortType::Out, nNewOut);
